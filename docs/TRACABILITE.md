...
| Exigence                                                  | Implémentation                                                                                   | Test                                                                  |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------- |
| Les trois équipes existent dès la proposition             | `ensureTeams` appelée par `createProposal`, migration `0031` pour les propositions déjà ouvertes | `league-team-choice.test.ts` — « naît avec ses trois équipes, vides » |
| On rejoint l'équipe de son choix                          | `proposals.chooseTeam`, rang d'équipe et non identifiant de ligne                                | même fichier — « l'équipe de son choix »                              |
| Rejoindre la séance et l'équipe d'un geste                | `joinProposal(…, teamIndex)`, avant le comptage qui déclenche le tirage                          | même fichier — « du même geste »                                      |
| Toucher une place ailleurs, c'est y aller                 | `choosePitchSlot(…, teamIndex)`, une seule transaction                                           | même fichier — « c'est la rejoindre »                                 |
| Une équipe complète est refusée, et le refus dit où aller | `seatInTeam`, message nommant les équipes où il reste de la place                                | même fichier — « le refus dit où aller »                              |
| Le tirage ne touche pas à ceux qui ont choisi             | `completeTeams`, appelée par `composeTeams`                                                      | même fichier — « ne touche pas à ceux qui ont choisi »                |
| Le tirage rapproche les trois équipes                     | un tour ne sert que les équipes les moins remplies, la plus faible en premier                    | `teams.test.ts` — 9 tests, écart ≤ 6 sur 50 graines                   |
| Deux séances identiques donnent des équipes différentes   | chapeau mélangé, graine = identifiant de session                                                 | `teams.test.ts` — « ne donnent pas les mêmes équipes »                |
| Quitter la séance libère sa place                         | suppression de l'appartenance dans `leaveProposal`                                               | `league-team-choice.test.ts` — « libère sa place »                    |
| Une séance qui se défait garde les équipes choisies       | `team_members.chosen`, `clearDraw` ne rend que le tirage                                         | même fichier — « garde les équipes choisies »                         |
| Le paiement ne déplace personne                           | aucun chemin de recomposition au paiement                                                        | même fichier — « ne déplace personne »                                |
| Les modes où le camp se choisit ignorent cette route      | `playersChooseTeam` absent, refus explicite                                                      | même fichier — « ne connaît pas cette route »                         |

## La forme du terrain d'un club et d'un tournoi (CLUB-003)

| Exigence                                          | Implémentation                                                  | Test                                               |
| ------------------------------------------------- | --------------------------------------------------------------- | -------------------------------------------------- |
| Un club choisit sa forme                          | `squads.formation`, posée par `setLineup`                       | `club-formations.test.ts` — « passe en carré »     |
| Une place que la forme ne dessine pas est refusée | `isPitchSlot` côté serveur, avant l'écriture                    | même fichier — « est refusée »                     |
| Une forme inconnue à cinq est refusée             | `isFormation(5, …)`                                             | même fichier — « forme inconnue »                  |
| Recomposer sans nommer la forme la garde          | `formation` absent ≠ `null`                                     | même fichier — « reste quand on recompose »        |
| Effacer la composition ne défait pas la forme     | `clearLineup` ne touche pas `squads.formation`                  | même fichier — « ne défait pas la forme »          |
| Les places sortent dans l'ordre du terrain        | `lineupSlotsFor`, du but à la pointe                            | même fichier — « l'ordre du terrain »              |
| Une feuille de tournoi peut différer du club      | `tournament_entries.formation`, sinon celle du club             | `tournament-formations.test.ts`                    |
| Le raccourci reprend la forme avec les joueurs    | `fillEntryFromSquadLineup` passe `readFormation`                | même fichier                                       |
