...
describe("cause technique d'une erreur interne", () => {
  ...
  it("remonte le message du pilote, pas la requête", () => {
    const cause = describeCause(
      drizzleError(
        "Unknown column 'players.position' in 'field list'",
        "ER_BAD_FIELD_ERROR",
        1054,
      ),
    );
  ...
  it("borne la longueur du message", () => {
    const cause = describeCause(
      drizzleError("x".repeat(5000), "ER_UNKNOWN", 9999),
    );
  ...
  it("reconnaît un schéma en retard sur le code", () => {
    expect(
      isSchemaDriftError(
        drizzleError("Unknown column 'a'", "ER_BAD_FIELD_ERROR", 1054),
      ),
    ).toBe(true);
    expect(
      isSchemaDriftError(
        drizzleError("Table 'x' doesn't exist", "ER_NO_SUCH_TABLE", 1146),
      ),
    ).toBe(true);
    expect(
